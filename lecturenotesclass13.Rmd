...
-its a net. if someone is 30 you might not guess right but if you say between they are 25-35 you will be right. A range of plausible values you have a better chance of getting that value. 
...
to be left handed?
about 5 
Would you be surprised if only 3 of them were left handed? 

no, not really. 

Would you be surprised if 40 of them were left handed?
that would suggest a lot of variability. Could suggest you had a bias
...
population %>% 
  ...
  theme_bw()
```
rnorm is telling us that its going to be centered at 20.8 with a deviation of 5.8
...
set.seed(1902370)
```
setting the seed is important means that the same random process for everyone. 
...
- Parameter of interest: mean price of those apartments 
...
visualize bootstap mean 
```{r visualize_means_manhattan}
boot_means %>% 
  ggplot(aes(x = stat)) +
  geom_histogram(binwidth = 50)
```
Bell shape, some a cheaper some are more expensive. 
...
```
there is 95% of points between these two points. In repeated samples we are 95% confident contains the the true mean. This is not saying WHERE it is located. 
...
```
you could also use function get_ci it will get the confidence interval. 
...
boot_prop <- mice %>% 
  ...
  calculate(stat = "prop")
```
gives us the proportion of mice that died when you similate the rounds. 

```{r ci_mice}
ci_mice_died <- get_ci(boot_prop, level = .95)
ci_mice_died
```
we are 95% confident that the proportion of mice that died is between 69 and 79.3 percent, the true proportion will be in this interval. 
...
```
this visualization is showing us us the shaded confidence interval. 
